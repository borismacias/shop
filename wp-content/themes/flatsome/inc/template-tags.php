...
function flatsome_add_to_cart_dropdown( $fragments ) {
	...
	<a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" class="cart-link">
					<strong class="cart-name hide-for-small"><?php _e('Cart', 'es_CL'); ?></strong> 
							...
							<div class="nav-dropdown">
                                        ...
                                        if ($_product->exists() && $cart_item['quantity']>0) :  ?>  
                                      	...
                                      	<div class="row mini-cart-item collapse">
                                      		<div class="small-2 large-2 columns">
                                      			<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s"><span class="icon-close"></span></a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), __('Remove this item', 'es_CL') ), $cart_item_key ); ?>
                                      		</div>
                                      		<div class="small-7 large-7 columns"><?php 
                                                 ...
                                                 echo '<div class="cart_list_product_quantity">'.__('Quantity', 'es_CL').': '.$cart_item['quantity'].'</div>';
                                ...
                                ?>
                                    ...
                                    <div class="minicart_total_checkout">                                        
                                        <?php _e('Cart Subtotal', 'es_CL'); ?><span><?php echo $woocommerce->cart->get_cart_total(); ?></span>                                   
                                    </div>
                                    
                                    <a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" class="button expand uppercase"><?php _e('View Cart', 'es_CL'); ?></a>   
                                    
                                    <a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="button secondary expand uppercase"><?php _e('Proceed to Checkout', 'es_CL'); ?></a>
                                    
                                    <?php                                        
                                    else: echo '<p class="empty">'.__('No products in the cart.','es_CL').'</p>'; endif;                                    
                                ?>                                                                        
  ...
  function ProductShowReviews(){
            if ( comments_open() ) {
                ...
                if ( $count > 0 ) {
                    ...
                    echo '<a href="#tab-reviews" class="scroll-to-reviews"><div class="star-rating tip-top" data-tip="'.$count.' review(s)"><span style="width:'.($average*16).'px"><span itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating" class="rating"><span itemprop="ratingValue">'.$average.'</span><span itemprop="reviewCount" class="hidden">'.$count.'</span></span> '.__('out of 5', 'es_CL').'</span></div></a>';
